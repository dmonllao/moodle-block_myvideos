...
function myvideos_show_video_actions($videodata, $courseid) {
    ...
    if ($USER->id == $videodata->userid || $videodata->public == 2) {
        ...
        // Download link checking if it's a video or a link
        if ($videodata->link == 0) {
            $actions[] = '<a href="'.$CFG->wwwroot.'/blocks/myvideos/getfile.php?videoid='.$videodata->id.'">'.get_string("download", "block_myvideos").'</a>';
        }

        // Embed
        $actions[] = '<a href="#" id="id_display_embed">'.get_string("getembedcode", "block_myvideos").'</a>';
    }
    ...
    $PAGE->requires->yui_module('moodle-block_myvideos-embed', 'M.block_myvideos.init_embed_code', null, null, true);
...
function myvideos_add_view($videodata) {
    ...
    $DB->update_record('myvideos_video', $tmpobj);
}
...
function myvideos_show_comments($videodata) {
    ...
    $comments = $DB->get_records('myvideos_video_comment', array('videoid' => $videodata->id), "timeadded DESC");
...
function myvideos_show_comment($comment) {
    ...
    // Picture
    $userpicture = $DB->get_record('user', array('id' => $comment->userid));

    echo '<tr class="header"><td class="picture left">';
    echo $OUTPUT->user_picture($userpicture);
    echo '</td>';
