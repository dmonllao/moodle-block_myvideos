...
class myvideos_actionable {
    ...
    function _get_video_tags($videoid) {
        ...
        $sql = "SELECT k.id, k.keyword 
                FROM {$CFG->prefix}myvideos_video_tag t 
                JOIN {$CFG->prefix}myvideos_video_keyword k ON k.id = t.keywordid 
                WHERE t.videoid = '$videoid' ORDER BY t.id";
        
        if (!$keywords = get_records_sql($sql)) {
            return '';
        }
    ...
    function _update_video_tags($videoid, $tagsstring) {
        ...
        $videotags = get_records_sql($videotagssql);
        ...
        foreach ($keywords as $keyword) {
            ...
            // Getting keyword id
            if (!$keywordid = get_field('myvideos_video_keyword', 'id', 'keyword', $keyword)) {
                
                // Inserting tag
                $keywordobj->keyword = addslashes($keyword);
                if (!$keywordid = insert_record('myvideos_video_keyword', $keywordobj)) {
                    print_error('errorinserting', 'block_myvideos');
                }
            }
            
            // Adding new video tags
            if (!$tagid = get_field('myvideos_video_tag', 'id', 'videoid', $videoid, 'keywordid', $keywordid)) {
                
                $tagdata->videoid = $videoid;
                $tagdata->keywordid = $keywordid;
                
                if (!$tagid = insert_record('myvideos_video_tag', $tagdata)) {
                    print_error('errorinserting', 'block_myvideos');
                }
            }
        ...
        if (!empty($videotags)) {
            foreach ($videotags as $videotag) {
                delete_records('myvideos_video_tag', 'id', $videotag->id);
            }
        }
